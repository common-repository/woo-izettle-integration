...
if (!class_exists('WC_Settings_Page_iZettle', false)) {
    ...
    class WC_Settings_Page_iZettle extends WC_Settings_Page
    {
        ...
        public function __construct()
        {
            ...
            add_action('woocommerce_settings_' . $this->id, array($this, 'refresh_from_error'), 6);
        ...
        public function authorize_processing()
        {
            ...
            if (array_key_exists('organization_uuid', $_REQUEST)) {
                ...
                if (wc_string_to_bool(get_option('izettle_lock_organization_uuid')) && get_option('izettle_organization_uuid') != $_REQUEST['organization_uuid']) {
                    WC_IZ()->logger->add(sprintf('Trying to authorize with a different Zettle account with the uuid %s than the one the plugin is authorized with %s', $_REQUEST['organization_uuid'], get_option('izettle_organization_uuid')));
                    IZ_Notice::add(sprintf(__('This plugin is already authorized with a different Zettle account with the uuid %s', 'woo-izettle-integration'), get_option('izettle_organization_uuid')), 'error');
                    return;
                }
        ...
        public function refresh_from_error() {
            if (array_key_exists('zettle_refresh', $_REQUEST)) {
                IZ_Notice::clear();
                WC_IZ()->logger->add('Forced connection via error notice');
                ...
                try {
                    ...
                    IZ_Notice::add('Connection established with Zettle', 'success');
                ...
                //Redirect to the same page to remove the refresh from the URL
                wp_safe_redirect(admin_url('admin.php?page=wc-settings&tab=izettle&section=advanced'));
        ...
        public function get_izettle_settings()
        {
            ...
            if ('izettle_stocklevel_from_izettle_options' == $current_section) {
                ...
                //zettle_wpml_multi_currency_compatibility
                $settings[] = [
                    'title' => __('Better WPML multi currency support', 'woo-izettle-integration'),
                    'type' => 'checkbox',
                    'desc' => __('Will use a better method to sync WPML multi currency prices to Zettle', 'woo-izettle-integration'),
                    'default' => '',
                    'id' => 'zettle_wpml_multi_currency_compatibility',
                ];
                ...
                //izettle_process_giftcard_as_item
                $settings[] = [
                    'title' => __('Process used gift cards as items', 'woo-izettle-integration'),
                    'type' => 'checkbox',
                    'desc' => __('Process used Zettle gift cards as order items when importing Zettle purchases to WooCommerce. This will add the gift card as a line item in WooCommerce', 'woo-izettle-integration'),
                    'default' => '',
                    'id' => 'izettle_process_giftcard_as_item',
                ];
                ...
                //zettle_process_purchase_new_wc_customer
                $settings[] = [
                    'title' => __('Enable new customer creation', 'woo-izettle-integration'),
                    'type' => 'checkbox',
                    'desc' => __('Create a new customer in WooCommerce if the customer does not exist. This will only work if an email address is present in the comments.', 'woo-izettle-integration'),
                    'default' => '',
                    'id' => 'zettle_process_purchase_new_wc_customer',
                ];
                ...
                //zettle_remove_zettle_item_id
                $settings[] = [
                    'title' => __('Remove Zettle item ID from order item meta', 'woo-izettle-integration'),
                    'type' => 'checkbox',
                    'desc' => __('Remove the Zettle item ID from the order item meta.', 'woo-izettle-integration'),
                    'default' => '',
                    'id' => 'zettle_remove_zettle_item_id',
                ];
                ...
                if ('yes' === get_option('izettle_extra_admin')) {
                    ...
                    $settings[] = [
                        'title' => __('Lock Organization UUID', 'woo-izettle-integration'),
                        'type' => 'checkbox',
                        'desc' => __('When checked, the plugin will not allow changing to a different Zettle account.', 'woo-izettle-integration'),
                        'default' => '',
                        'id' => 'izettle_lock_organization_uuid',
                    ];
