                        ...
                        try {
                            ...
                            if ($file_data !== false) {
                                ...
                                if ($is_webp && $imageformat === IMAGETYPE_WEBP) {
                                    ...
                                    $file_data = $this->image_data_convert($image_path, 'webp');
                                    $imageformat = 'PNG';
                                } elseif ($is_avif && $imageformat === IMAGETYPE_AVIF) {
                                    WC_IZ()->logger->add(sprintf('get_image_keys (%s): Converting AVIF image %s to PNG', $product_id, $image_path));
                                    $file_data = $this->image_data_convert($image_path, 'avif');
                                    $imageformat = 'PNG';
                                } else {
                                    WC_IZ()->logger->add(sprintf('get_image_keys (%s): Image %s is not a WEBP or AVIF image', $product_id, $image_path));
                                }

                                if ($file_data === false) {
                                    WC_IZ()->logger->add(sprintf('get_image_keys (%s): Failed to convert image %s to PNG', $product_id, $image_path));
                                } else {
                                    WC_IZ()->logger->add(sprintf('get_image_keys (%s): Image %s converted to PNG', $product_id, $image_path));
