...
if (!class_exists('WC_iZettle_Integration_Purchase_Handler', false)) {
    ...
    class WC_iZettle_Integration_Purchase_Handler
    {
        ...
        public function get_commment_email($purchase) {
            foreach ($purchase->products as $purchase_product) {
                if (isset($purchase_product->comment)) {
                    $comments = explode(',', $purchase_product->comment);
                    foreach ($comments as $comment) {
        ...
        public function get_comment_phone($purchase) {
            foreach ($purchase->products as $purchase_product) {
                if (isset($purchase_product->comment)) {
                    $comments = explode(',', $purchase_product->comment);
                    foreach ($comments as $comment) {
        ...
        public function get_comment_name($purchase) {
            foreach ($purchase->products as $purchase_product) {
                if (isset($purchase_product->comment)) {
                    $comments = explode(',', $purchase_product->comment);
                    foreach ($comments as $comment) {
                        if (false === strpos($comment, '@') && false === strpos($comment, '#') && false === strpos($comment, '+')) {
                            return trim($comment);
                        }
                    }
                }
            ...
            if (!empty($woosb_ids)) {
                ...
                if (is_array($woosb_items) && count($woosb_items) > 0) {
                    ...
                    foreach ($woosb_items as $woosb_item) {
                        ...
                        if (!wc_string_to_bool(get_option('zettle_remove_zettle_item_id'))) {
        ...
        public function process_purchase_wc_order($post_id, $original_order_id = false)
        {
            try {
                ...
                if ($original_order_id || !($processed_timestamp = get_post_meta($post_id, '_processed_timestamp', true))) {
                    ...
                    $process_purchase_new_wc_customer = wc_string_to_bool(get_option('zettle_process_purchase_new_wc_customer'));
                    ...
                    if (!$process_purchase_order_id) {
                        ...
                        } else if ($process_purchase_new_wc_customer) {
                            $email = $this->get_commment_email($purchase);

                            $name = $this->get_comment_name($purchase);

                            if (!$email) {
                                WC_IZ()->logger->add(sprintf('process_purchase_wc_order: No email found on purchase'));
                            } else if (($user = get_user_by('email', $email))) {
                                $customer_id = $user->ID;
                                WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Customer id "%s" found on purchase using email %s', $customer_id, $email));
                            } else {
                                if ($name = $this->get_comment_name($purchase)) {
                                    ...
                                    $customer_user_name = $first_name . ($last_name ? '.' . $last_name : '');

                                    $customer_user_name = preg_replace('/[^a-zA-Z0-9@.]/', '', $customer_user_name);

                                    $new_customer_id = wc_create_new_customer($email, $customer_user_name, '', array(
                                        'first_name' => $first_name,
                                        'last_name' => $last_name                                    
                                    ));

                                    if ($new_customer_id instanceof WP_Error) {
                                        WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Error creating customer with email %s', $email));
                                    } else {
                                        WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Customer id "%s" created from purchase', $customer_id));
                                        $customer_id = $new_customer_id;
                                    }
                                } else {
                                    //Creating customer with just email
                                    WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Creating customer with email %s', $email));
                                    $customer_user_name = preg_replace('/[^a-zA-Z0-9@.]/', '', $email);

                                    $new_customer_id = wc_create_new_customer($email, $customer_user_name);
                    ...
                    foreach ($purchase->products as $purchase_product) {
                        ...
                        if (true == $purchase_product->libraryProduct) {
                            ...
                            try {
                                ...
                                WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Got product %s via uuid %s', $wc_product ? $wc_product->get_name() : 'no product', $purchase_product->productUuid));
                            ...
                            if (!$wc_product && isset($purchase_product->sku)) {
                                ...
                                WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Got product %s via sku %s', $wc_product ? $wc_product->get_name() : 'no product', $purchase_product->sku));
                            ...
                            if ($wc_product) {
                                ...
                                if (($parent_id = $wc_product->get_parent_id()) && ($parent = wc_get_product($parent_id)) && !$parent->is_type('grouped')) {
                                    ...
                                    WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Adding variation id %s to order with product id %s', $wc_product->get_id(), $parent_id));
                                    //Log SKU from WooCommerce product if existing
                                    if ($wc_product->get_sku()) {
                                        WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Adding sku %s to order', $wc_product->get_sku()));
                                    } else {
                                        WC_IZ()->logger->add(sprintf('process_purchase_wc_order: No sku found on product'));
                                    }

                                } else {
                                    wc_add_order_item_meta($order_item_id, '_product_id', $wc_product->get_id(), true);
                                    WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Adding simple product id %s to order', $wc_product->get_id()));
                                    //Log SKU from WooCommerce product if existing
                                    if ($wc_product->get_sku()) {
                                        WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Adding sku %s to order', $wc_product->get_sku()));
                                    } else {
                        ...
                        //if zettle_remove_zettle_item_id is set to false
                    ...
                    $gift_card_amount_processed = 0;

                    if (wc_string_to_bool(get_option('izettle_process_giftcard_as_item'))) {
                        ...
                        foreach ($purchase->payments as $payment) {
                            ...
                            if ($payment_type == 'GIFTCARD') {
                                WC_IZ()->logger->add(sprintf('process_purchase_wc_order: Gift card payment method found'));

                                $gift_card_uuid = $payment->details->giftcardUuid;

                                $zettle_gift_card_name = __('Zettle gift card', 'woo-izettle-integration');

                                $order_item_id = wc_add_order_item($order_id, array(
                                    'order_item_name' => $zettle_gift_card_name,
                                    'order_item_type' => 'line_item',
                                ));

                                wc_add_order_item_meta($order_item_id, 'izettle_gift_card_used_id', $gift_card_uuid, true);

                                wc_add_order_item_meta($order_item_id, '_qty', 1, true);
                                
                                wc_add_order_item_meta($order_item_id, '_line_subtotal', -1 * ($payment->amount / 100), true);
                                wc_add_order_item_meta($order_item_id, '_line_total', -1 * ($payment->amount / 100), true);

                                $gift_card_amount_processed += $payment->amount;
                            }
                        }
                    ...
                    $order->set_total(($purchase->amount - $gift_card_amount_processed) / 100);
