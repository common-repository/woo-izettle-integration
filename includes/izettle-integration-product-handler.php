...
if (!class_exists('WC_iZettle_Integration_Product_Handler', false)) {
    ...
    class WC_iZettle_Integration_Product_Handler
    {
        ...
        public function get_price($product, $tax_rate)
        {
            ...
            $price = $this->maybe_return_wpml_multi_currency_price($price, $product);
        ...
        private function maybe_return_wpml_multi_currency_price($price, $product) {

            if (!wc_string_to_bool(get_option('zettle_wpml_multi_currency_compatibility'))) {
                return $price;
            }

            if (!WC_Zettle_Helper::isWPMLMultiCurrencyEnabled()) {
                return $price;
            }
            ...
            if (get_woocommerce_currency() == $this->get_currency()) {
                ...
                WC_IZ()->logger->add(sprintf('maybe_return_wpml_multi_currency_price (%s): Using standard price %s', $product_id, $price));
                return $price;
            } else {
                $price = $woocommerce_wpml->multi_currency->prices->get_product_price_in_currency($product_id, $this->get_currency());
                WC_IZ()->logger->add(sprintf('maybe_return_wpml_multi_currency_price (%s): Using WPML multi currency price %s', $product_id, $price));
                return $price;
            }
                ...
                foreach ($parent->get_attributes('edit') as $attribute) {
                    ...
                    if ($attribute->is_taxonomy()) {
                        ...
                        foreach ($attribute->get_terms() as $option) {
                            ...
                            if (WC_Zettle_Helper::sanitize_attribute_string($option->slug) === WC_Zettle_Helper::sanitize_attribute_string($selected_value)) {
                                $value = substr($option->name, 0, 256);
                                $options[] = array(
                                    "name" => $attribute_name,
                                    "value" => $value,
                        ...
                        foreach ($attribute->get_options() as $option) {
                            if (WC_Zettle_Helper::sanitize_attribute_string($option) === WC_Zettle_Helper::sanitize_attribute_string($selected_value)) {
                                $value = substr($option, 0, 256);
                                $options[] = array(
                                    "name" => $attribute_name,
                                    "value" => $value,
