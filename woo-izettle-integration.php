...
class WC_iZettle_Integration
{
    ...
    public function generate_messages()
    {
        ...
        if ($connection_status == 'unauthorized' && !get_site_transient('izettle_did_show_unauthorized_info')) {
            ...
            $id = IZ_Notice::add($this->generate_expired_content(), 'warning', 'expired_subscription', true, false, true);
            ...
            $message = sprintf(__('WooCommerce Zettle Integration cannot connect to service. Try to <a href="%s">refresh the connection.</a> If you still experience problems contact hello@bjorntech.com', 'woo-izettle-integration'), get_admin_url(null, 'admin.php?page=wc-settings&tab=izettle&section=advanced&zettle_refresh=true'));
    ...
    public function generate_expired_content () {
        $heading = 'Your Zettle Integration subscription has expired!';

        $content = "Extend your subscription to enable full syncing capabilities again in our webshop";

        ob_start(); ?>
            <div class="zettle-notice-dashboard">
                <div class="zettle-notice-content">
                ...
                <p>
                <a 
                    target="_blank" href="<?php echo esc_url( 'https://www.bjorntech.com/product/woocommerce-izettle-integration-automatic-sync/?token=' . izettle_api()->get_organization_uuid() . '&utm_source=wp-izettle&utm_medium=plugin&utm_campaign=product' ); ?>"
                    class="button button-primary"><?php echo esc_html( 'Get subscription' ) ?></a>
                </p>
            </div>
			<?php
			return wp_kses_post( apply_filters( 'zettle_expired_display', ob_get_clean() ) );
    }
